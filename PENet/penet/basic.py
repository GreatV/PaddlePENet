...
#         # normalization
#         guide_sum = paddle.sum(paddle.abs(guide), axis=1, keepdim=True)
#         guide = paddle.divide(guide, guide_sum)
#         guide_mid = (1 - paddle.sum(guide, axis=1, keepdim=True)).unsqueeze(1)

#         # padding
#         weight_pad = [i for i in range(self.kernel_size * self.kernel_size)]
#         for t in range(self.kernel_size * self.kernel_size):
#             zero_pad = nn.ZeroPad2D(padding=0)
#             if self.kernel_size == 3:
#                 zero_pad = nn.ZeroPad2D(padding=get_pads(3)[t])
#             elif self.kernel_size == 5:
#                 zero_pad = nn.ZeroPad2D(padding=get_pads(5)[t])
#             elif self.kernel_size == 7:
#                 zero_pad = nn.ZeroPad2D(padding=get_pads(7)[t])
...
#             [weight_pad[t] for t in range(self.kernel_size * self.kernel_size)],
#             axis=1
#         )
#         return guide_weight
...
class CSPNGenerate(nn.Layer):
    def __init__(self, in_channels, kernel_size):
        ...
        self.generate = nn.Sequential(
            ...
            nn.BatchNorm2D(self.kernel_size * self.kernel_size - 1),
        )
...
class CSPNGenerateAccelerate(nn.Layer):
    ...
    def forward(self, feature):
        ...
        guide_sum = paddle.sum(paddle.abs(guide), axis=1, keepdim=True)
        guide = paddle.divide(guide, guide_sum)
        guide_mid = 1 - paddle.sum(guide, axis=1, keepdim=True)
