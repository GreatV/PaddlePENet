...
class Result(object):
    ...
    def evaluate(self, output, target, photometric=0):
        ...
        target_mm = 1000.0 * target[valid_mask]
        abs_diff = (output_mm - target_mm).abs()
        self.mse = float(paddle.pow(x=abs_diff, y=2).mean())
        ...
        maxRatio = paddle.maximum(x=output_mm / target_mm, y=target_mm / output_mm)
        self.delta1 = float((maxRatio < 1.25).astype(dtype="float32").mean())
        self.delta2 = float((maxRatio < 1.25**2).astype(dtype="float32").mean())
        self.delta3 = float((maxRatio < 1.25**3).astype(dtype="float32").mean())
        self.data_time = 0
        self.gpu_time = 0
        err_log = paddle.log(x=target[valid_mask]) - paddle.log(x=output[valid_mask])
        ...
        inv_output_km = (0.001 * output[valid_mask]) ** -1
        inv_target_km = (0.001 * target[valid_mask]) ** -1
        abs_inv_diff = (inv_output_km - inv_target_km).abs()
        self.irmse = math.sqrt(paddle.pow(x=abs_inv_diff, y=2).mean())
...
class AverageMeter(object):
    ...
    def update(self, result, gpu_time, data_time, n=1):
        ...
        if self.time_stable is True and self.time_stable_counter > 0:
