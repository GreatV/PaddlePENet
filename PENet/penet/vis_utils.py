...
def save_image_torch(rgb, filename):
    rgb = validcrop(rgb)
    rgb = np.squeeze(rgb[0, ...].detach().cpu().numpy())
    rgb = np.transpose(rgb, (1, 2, 0))
