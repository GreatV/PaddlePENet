...
def _is_tensor_image(img):
    return paddle.is_tensor(x=img) and img.ndimension() == 3
...
def adjust_hue(img, hue_factor):
    ...
    if not -0.5 <= hue_factor <= 0.5:
        raise ValueError("{} is not in [-0.5, 0.5].".format(hue_factor))
    if not _is_pil_image(img):
        raise TypeError("img should be PIL Image. Got {}".format(type(img)))
    input_mode = img.mode
...
class ToTensor(object):
    ...
    def __call__(self, img):
        ...
        if isinstance(img, np.ndarray):
            if img.ndim == 3:
                img = paddle.to_tensor(data=img.transpose((2, 0, 1)).copy())
            elif img.ndim == 2:
                img = paddle.to_tensor(data=img.copy())
            else:
                raise RuntimeError(
...
class NormalizeNumpyArray(object):
    ...
    Given mean: ``(M1,...,Mn)`` and std: ``(M1,..,Mn)`` for ``n`` channels,
    this transform
...
class NormalizeTensor(object):
    ...
    def __call__(self, tensor):
        ...
        for t, m, s in zip(tensor, self.mean, self.std):
            # t.sub_(m).div_(s)
            paddle.subtract(t, m, out=t)
            paddle.divide(t, s, out=t)
        return tensor
