...
"""
    ...
    def conditional_save_img_comparison(
        ...
        if mode == "val" or mode == "eval":
            skip = 100
            if i == 0:
                self.img_merge = vis_utils.merge_into_row(
    ...
    def conditional_save_pred(self, mode, i, pred, epoch):
        if ("test" in mode or mode == "eval") and self.args.save_pred:
            ...
            img = paddle.squeeze(x=pred.data.cpu()).numpy()
...
def adjust_learning_rate(lr_init, optimizer, epoch, args):
    ...
    lr = lr_init
    if args.network_model == "pe" and args.freeze_backbone is False:
        if epoch >= 10:
            lr = lr_init * 0.5
        if epoch >= 20:
            lr = lr_init * 0.1
...
def save_checkpoint(state, is_best, epoch, output_directory):
    ...
    paddle.save(obj=state, path=checkpoint_filename, protocol=4)
    if is_best:
...
def get_folder_name(args):
    ...
    return os.path.join(
        args.result,
        "input={}.criterion={}.lr={}.bs={}.wd={}.jitter={}.time={}".format(
            args.input,
            args.criterion,
            args.lr,
            args.batch_size,
...
avgpool = paddle.nn.AvgPool2D(kernel_size=2, stride=2, exclusive=False)
